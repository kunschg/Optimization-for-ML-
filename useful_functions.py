...
def generate_dataset(nsamples, range_uniform=1, sigma=0.0):
    """
...
def gradient_descent(
    A_train, Y_train, nb_of_iterations, step_size, ridge_parameter, add_bias
):
    """
    ...
    if add_bias == True:
        ...
        for i in range(nb_of_iterations):
            ...
            loss_function_train = np.linalg.norm(
                np.dot(A_train_b, x) - Y_train
            ) ** 2 + ridge_parameter * x.transpose().dot(x)
            loss_function_array.append(loss_function_train)

            grad = (
                2 * A_train_b.T.dot(np.dot(A_train_b, x) - Y_train)
                + 2 * ridge_parameter * x
            )
            x = x - step_size * grad
        ...
        for i in range(nb_of_iterations):
            ...
            loss_function_train = np.linalg.norm(
                np.dot(A_train, x) - Y_train
            ) ** 2 + ridge_parameter * x.transpose().dot(x)
            loss_function_array.append(loss_function_train)

            grad = (
                2 * A_train.T.dot(np.dot(A_train, x) - Y_train)
                + 2 * ridge_parameter * x
            )
            x = x - step_size * grad
...
def plot_convergence_rate(A_train, Y_train, niter, ridge_parameter):
    ...
    xopt = np.linalg.solve(
        A_train.transpose().dot(A_train) + ridge_parameter * np.eye(D),
        A_train.transpose().dot(Y_train),
    )
    plt.clf
    ...
    for istep in np.arange(0, len(step_size_mult)):
        ...
        e = np.log10(
            flist
            - np.linalg.norm(np.dot(A_train, xopt) - Y_train) ** 2
            - ridge_parameter * xopt.transpose().dot(xopt)
            + 1e-20
        )
...
def plot_evolution_on_test_error(A_train, Y_train, A_test, Y_test, start, stop, num):
    ...
    for i in np.arange(0, num):
        ...
        x = np.linalg.solve(
            A_train.transpose().dot(A_train) + Lambda * np.eye(p),
            A_train.transpose().dot(Y_train),
        )
...
def plot_evolution_on_test_error_with_bias(
    A_train, Y_train, A_test, Y_test, start, stop, num
):
    """
    Code from Gabriel Peyré
    """
    ...
    for i in np.arange(0, num):
        Lambda = lambda_list[i]
        x = np.linalg.solve(
            A_train.transpose().dot(A_train) + Lambda * np.eye(p + 1),
            A_train.transpose().dot(Y_train),
        )
        X[:, i] = x.flatten()  # bookkeeping
        E[i] = np.linalg.norm(A_test.dot(x) - Y_test) / np.linalg.norm(Y_test)
...
def gradient_descent_with_momentum(
    A_train, Y_train, nb_of_iterations, step_size, gamma
):
...
def plot_convergence_rate_with_momentum(A_train, Y_train, niter, step_size):
    ...
    xopt = np.linalg.solve(
        A_train.transpose().dot(A_train), A_train.transpose().dot(Y_train)
    )
    plt.clf
    ...
    for igamma in np.arange(0, len(gamma_mult)):
        ...
        e = np.log10(
            flist - np.linalg.norm(np.dot(A_train, xopt) - Y_train) ** 2 + 1e-20
        )
...
def plot_iterates_l2_2D(iterates, proj_iterates, R, with_set):
    ...
    ax.plot(
        proj_iterates[0, :], proj_iterates[1, :], "gx-", label="Iterates GD Projected"
    )
    ax.legend()
...
def plot_iterates_l1_2D(iterates, proj_iterates, R, with_set):
    ...
    if with_set:
        square = plt.Rectangle(
            (0, -R), R * np.sqrt(2), R * np.sqrt(2), angle=45, color="k", fill=False
        )
        ax.add_patch(square)
